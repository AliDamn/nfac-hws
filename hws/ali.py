...
class Matrix:
    ...
    def multiply(self, other):
        if self.cols != other.rows:
            raise ValueError
        result = [[sum(self.matrix[i][k] * other.matrix[k][j] for k in range(self.cols)) for j in range(other.cols)] for
...
class Triangle:
    ...
    def area(self):
        ...
        return math.sqrt(s * (s - self.side_a) * (s - self.side_b) * (s - self.side_c))  # Heron's formula
